...
Deno.serve(async (req: Request) => {
  ...
  try {
    const { origin, tokenScheme = "sixTOKENfour" } = await req.json();

    const tokenExID = Deno.env.get("TOKENEX_ID") || "7320744805319527";
    const clientSecretKey = Deno.env.get("TOKENEX_API_KEY");

    if (!clientSecretKey) {
      console.error("TOKENEX_API_KEY not configured");
      return new Response(
        JSON.stringify({
          success: false,
